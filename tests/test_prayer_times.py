...
from azan_prayer_times import PrayerTimeCalculator, CityCoordinates
...
class TestPrayerTimeCalculator:
    ...
    def test_calculate_times_contains_all_prayers(self):
        ...
        assert required.issubset(result.keys()), f"Missing prayers: {required - result.keys()}"
    ...
    def test_prayer_times_logical_order(self):
        ...
        assert fajr < dhuhr, f"Fajr {result['fajr']} must be before Dhuhr {result['dhuhr']}"
        assert dhuhr < asr, f"Dhuhr {result['dhuhr']} must be before Asr {result['asr']}"
        assert asr < maghrib, f"Asr {result['asr']} must be before Maghrib {result['maghrib']}"
        assert maghrib < isha, f"Maghrib {result['maghrib']} must be before Isha {result['isha']}"

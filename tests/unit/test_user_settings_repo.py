...
async def test_list_with_notifications_filters(repo):
    ...
    await repo.upsert(
        UserSettings(user_id=1, city="مكة المكرمة", timezone=3, notifications_on=True)
    )
    await repo.upsert(
        UserSettings(user_id=2, city="القاهرة", timezone=2, notifications_on=False)
    )
...
async def test_upsert_is_an_update(repo):
    ...
    await repo.upsert(
        UserSettings(user_id=1, city="جدة", timezone=3, notifications_on=False)
    )
    got = await repo.get(1)
    assert got.city == "جدة"
...
async def test_invalid_enabled_prayers_falls_back_to_default(repo):
    ...
    db = repo._db
    await db.execute("""INSERT INTO user_settings (user_id, city, enabled_prayers)

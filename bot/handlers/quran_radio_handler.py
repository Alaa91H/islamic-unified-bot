...
def _radio_keyboard(
    ...
    kb = [
        [
            ...
            InlineKeyboardButton(
                "▶️" if is_paused else "⏸️",
                callback_data=f"rdo:{'resume' if is_paused else 'pause'}:{chat_id}",
            ),
            InlineKeyboardButton("⏭️", callback_data=f"rdo:next:{chat_id}"),
        ],
        [
            InlineKeyboardButton(
                "✅ عشوائي" if shuffle_on else "🔀 عشوائي",
                callback_data=f"rdo:shuffle:{chat_id}",
            ),
            InlineKeyboardButton("🎙️ القارئ", callback_data=f"rdo:reciters:{chat_id}"),
        ],
        [
            InlineKeyboardButton(
                f"🎚️ {q_label}", callback_data=f"rdo:quality:{chat_id}"
            ),
            InlineKeyboardButton(
                "⏹️ إيقاف الراديو", callback_data=f"rdo:stop:{chat_id}"
            ),
        ],
        [
    ...
    return markup_with_bottom_controls(
        ...
        back_label="🎛️ التحكم",
        home_callback=None,
        close_callback=f"rdo:close:{chat_id}",
    )
    ...
    for key, info in QURANIC_RECITERS.items():
        kb.append(
            [
                InlineKeyboardButton(
                    f"🎙️ {info['name']}",
                    callback_data=f"rdo:setreciter:{chat_id}:{key}",
                )
            ]
        )
    return markup_with_bottom_controls(
        kb,
        back_callback=f"rdo:panel:{chat_id}",
        home_callback=None,
        close_callback=f"rdo:close:{chat_id}",
    )
...
def register(app, deps) -> None:
    ...
    async def radio_cmd(client, message):
        ...
        if args and args[0] == "stop":
            ...
            await message.reply_text(
                "✅ تم إيقاف راديو القرآن" if ok else "❌ لا يوجد راديو نشط"
            )
            return
        ...
        if ok:
            ...
            await message.reply_text(
                "❌ فشل بدء الراديو. تأكد من وجود مكالمة صوتية نشطة."
            )
    ...
    async def radio_control(client, cq: CallbackQuery):
        ...
        from bot.services.quran_radio import AUDIO_QUALITY_LABELS, AUDIO_QUALITY_OPTIONS
        ...
        if cq.message.chat.id != chat_id:
            await cq.answer(
                "❌ هذا التحكم خاص بالمجموعة التي بدأ منها الراديو", show_alert=True
            )
            return
        ...
        if action == "prev":
            ...
            await cq.message.edit_text(
                "🎙️ **اختر القارئ لراديو القرآن:**",
                reply_markup=_reciters_keyboard(chat_id),
            )
            await cq.answer()
            return
            ...
            await cq.answer(
                f"🎙️ تم تغيير القارئ إلى {QURANIC_RECITERS[reciter_key]['name']}"
                if ok
                else "❌ فشل"
            )
        elif action == "quality":
            ...
            idx = (
                AUDIO_QUALITY_OPTIONS.index(current)
                if current in AUDIO_QUALITY_OPTIONS
                else 2
            )
            next_q = AUDIO_QUALITY_OPTIONS[(idx + 1) % len(AUDIO_QUALITY_OPTIONS)]
            ok = await radio_service.set_quality(chat_id, next_q)
            await cq.answer(
                f"🎚️ الجودة: {AUDIO_QUALITY_LABELS.get(next_q, next_q)}"
                if ok
                else "❌ فشل"
            )
...
async def _show_panel(message, chat_id, radio_service):
    ...
    text = _build_radio_text(
        surah_num, surah_name, reciter_name, dur, state.shuffle, state.paused
    )
    await message.reply_text(

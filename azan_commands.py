...
from azan_manager import AzanScheduler, AzanStreamer, AzanNotificationManager, NotificationType, UserAzanSettings
...
async def register_azan_commands(app: Client):
    ...
    async def azan_setup(client: Client, message: Message):
        ...
        for i in range(0, len(city_list), 2):
            ...
            if i < len(city_list):
                city = city_list[i]
                row.append(InlineKeyboardButton(city, callback_data=f"azan_select_city:{city}"))
    ...
    async def select_city_handler(client: Client, callback_query: CallbackQuery):
        ...
        keyboard = [
            [
                InlineKeyboardButton("🇸🇦 كراتشي", callback_data=f"azan_select_method:{city}:karachi"),
                InlineKeyboardButton("🇸🇦 أم القرى", callback_data=f"azan_select_method:{city}:makkah"),
            ],
            [
                InlineKeyboardButton("🇺🇸 ISNA", callback_data=f"azan_select_method:{city}:isna"),
                InlineKeyboardButton("🇪🇬 مصر", callback_data=f"azan_select_method:{city}:egypt"),
            ],
            [
                InlineKeyboardButton("🇩🇿 الجزائر", callback_data=f"azan_select_method:{city}:algiers"),
                InlineKeyboardButton("🇦🇪 دبي", callback_data=f"azan_select_method:{city}:dubai"),
            ],
        ]
    ...
    async def select_method_handler(client: Client, callback_query: CallbackQuery):
        ...
        keyboard = [
            [InlineKeyboardButton("المشاهدة", callback_data=f"azan_view_settings:{user_id}")],
        ]
    ...
    async def azan_settings(client: Client, message: Message):
        ...
        keyboard = [
            [InlineKeyboardButton("🔔 إعدادات التنبيهات", callback_data="azan_notification_settings")],
            [InlineKeyboardButton("🎵 إعدادات البث", callback_data="azan_stream_settings")],
            ...
            [InlineKeyboardButton("📐 تغيير طريقة الحساب", callback_data="azan_change_method")],
    ...
    async def notification_settings_handler(client: Client, callback_query: CallbackQuery):
        """معالج إعدادات التنبيهات"""
        user_id = callback_query.from_user.id
        settings = azan_scheduler.get_user_settings(user_id)
        ...
        keyboard = [
            [InlineKeyboardButton(f"التنبيهات {notif_status}", 
                                 callback_data="azan_toggle_notifications")],
            [InlineKeyboardButton(f"المقدمة {prelude_status}", 
                                 callback_data="azan_toggle_prelude")],
            [InlineKeyboardButton("⏱️ وقت المقدمة", callback_data="azan_set_prelude_time")],
    ...
    async def toggle_notifications_handler(client: Client, callback_query: CallbackQuery):
        """معالج تفعيل/تعطيل التنبيهات"""
        user_id = callback_query.from_user.id
        settings = azan_scheduler.get_user_settings(user_id)
        ...
        await callback_query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"التنبيهات {'✅ مفعلة' if new_status else '❌ معطلة'}", 
                                     callback_data="azan_toggle_notifications")],
                [InlineKeyboardButton(f"المقدمة {'✅ مفعلة' if settings.prelude_enabled else '❌ معطلة'}", 
                                     callback_data="azan_toggle_prelude")],
                [InlineKeyboardButton("⏱️ وقت المقدمة", callback_data="azan_set_prelude_time")],
                [InlineKeyboardButton("↩️ عودة", callback_data="azan_settings_menu")],
            ])
    ...
    async def toggle_prelude_handler(client: Client, callback_query: CallbackQuery):
        ...
        await callback_query.edit_message_text(
            ...
            f"وقت المقدمة: {settings.prelude_time} دقيقة",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"التنبيهات {'✅ مفعلة' if settings.notification_enabled else '❌ معطلة'}", 
                                     callback_data="azan_toggle_notifications")],
                [InlineKeyboardButton(f"المقدمة {'✅ مفعلة' if new_status else '❌ معطلة'}", 
                                     callback_data="azan_toggle_prelude")],
                [InlineKeyboardButton("⏱️ وقت المقدمة", callback_data="azan_set_prelude_time")],
                [InlineKeyboardButton("↩️ عودة", callback_data="azan_settings_menu")],
            ])
    ...
    async def stream_settings_handler(client: Client, callback_query: CallbackQuery):
        ...
        keyboard = [
            [InlineKeyboardButton(f"البث {stream_status}", 
                                 callback_data="azan_toggle_stream")],
            [InlineKeyboardButton("⏹️ توقيت الإيقاف", callback_data="azan_set_stop_before")],
            [InlineKeyboardButton("📻 مدة البث", callback_data="azan_set_stream_duration")],
            [InlineKeyboardButton("↩️ عودة", callback_data="azan_settings_menu")],
        ]
    ...
    async def toggle_stream_handler(client: Client, callback_query: CallbackQuery):
        ...
        await callback_query.edit_message_text(
            ...
            f"مدة البث: {settings.stream_duration} ثانية",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"البث {'✅ مفعل' if new_status else '❌ معطل'}", 
                                     callback_data="azan_toggle_stream")],
                [InlineKeyboardButton("⏹️ توقيت الإيقاف", callback_data="azan_set_stop_before")],
                [InlineKeyboardButton("📻 مدة البث", callback_data="azan_set_stream_duration")],
                [InlineKeyboardButton("↩️ عودة", callback_data="azan_settings_menu")],
    ...
    async def azan_next(client: Client, message: Message):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("📅 أوقات اليوم", callback_data="azan_view_all_times")],
        ]
...
__all__ = ['register_azan_commands', 'azan_scheduler', 'azan_streamer', 'notification_manager']
